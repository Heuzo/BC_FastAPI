...
def auth_and_token(user: User, response: Response):
    ...
    if (user_data_from_db is not None) and (user.password == user_data_from_db.password):
...
def read_jwt_token(token: str = Depends(oauth2_scheme), claim: str | None = None,) -> str:

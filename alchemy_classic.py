from sqlalchemy import create_engine, select, Table, Column, Integer, String, Boolean, MetaData, ForeignKey

# Метадата - информация о данных в ДБ 
meta = MetaData()

# Создаем таблицы 
todo = Table('todo', meta,
...
engine = create_engine('postgresql+psycopg2://postgres:vivaldi2101@127.0.0.1:5432/fastapi', echo=True)
meta.create_all(engine) # или todo.create(engine) 

conn = engine.connect()

select_todo_all = todo.select()
insert_todo_query = todo.insert().values(title='Hello from alchemy', description='Alchemy description', completed=True)
